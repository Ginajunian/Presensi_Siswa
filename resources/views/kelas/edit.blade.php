<x-app-layout>
    ...
    <div class="max-w-xl mx-auto">
        <x-card>
            <form action="{{ route('kelas.update', $kela) }}" method="POST" class="space-y-4">
                @csrf
                @method('PUT')

                <div>
                    <x-input-label for="nama_kelas" value="Nama Kelas" />
                    <x-text-input id="nama_kelas" name="nama_kelas" type="text" value="{{ old('nama_kelas', $kela->nama_kelas) }}" class="w-full" />
                ...
                <div>
                    ...
                    <x-select id="wali_kelas_id" name="wali_kelas_id" class="w-full">
                        ...
                        @foreach ($guruList as $guru)
                            <option value="{{ $guru->id }}" @selected(old('wali_kelas_id', $kela->wali_kelas_id) == $guru->id)>{{ $guru->nama }}</option>
                        @endforeach
                    </x-select>
