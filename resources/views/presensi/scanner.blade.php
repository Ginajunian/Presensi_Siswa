    ...
    <div class="max-w-6xl mx-auto space-y-4">
        ...
        <x-card>
            <div class="flex justify-between items-center mb-3">
                <p class="text-sm text-gray-500">Arahkan kamera ke QR Code kartu siswa.</p>
            ...
            <details class="mt-4">
                <summary class="text-sm text-gray-400 cursor-pointer hover:text-gray-600">
                    Kamera bermasalah? Input manual di sini
                </summary>
                <div class="flex gap-2 mt-3">
                    <x-text-input type="text" id="qr-input" placeholder="Paste/ketik qr_token siswa..."
                        class="flex-1" />
                    <button id="btn-scan-manual"
                ...
                <table class="w-full text-left text-sm">
                    <thead>
                        <tr class="text-gray-400 text-xs uppercase tracking-wide border-b border-gray-100">
                            ...
                            @if ($sesi === 'masuk')
                                <th class="px-5 py-3 font-medium">Tandai Manual</th>
                            @endif
                        </tr>
                    </thead>
                    <tbody id="daftar-siswa">
                        @forelse ($siswaList as $siswa)
                            @php
                                ...
                                $jamLabel = $sesi === 'masuk'
                                    ? ($p && $p->waktu_masuk ? \Carbon\Carbon::parse($p->waktu_masuk)->format('H:i') : '-')
                                    : ($p && $p->waktu_pulang ? \Carbon\Carbon::parse($p->waktu_pulang)->format('H:i') : '-');
                            ...
                            <tr class="border-t border-gray-100 hover:bg-gray-50/60 transition"
                                ...
                                <td class="px-5 py-3 jam-cell text-gray-500">{{ $jamLabel }}</td>
                                @if ($sesi === 'masuk')
                                    <td class="px-5 py-3">
                                        <select
                        ...
                        @empty
                            <tr>
                                <td colspan="{{ $sesi === 'masuk' ? 4 : 3 }}" class="px-5 py-8 text-center text-gray-400">
    ...
    <script>
        ...
        function updateBaris(nama, statusText, jamText) {
            ...
            if (baris) {
                ...
                statusCell.className = 'status-cell inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ' + badgeClassFor(statusText);
                baris.querySelector('.jam-cell').textContent = jamText;
